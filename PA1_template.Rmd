...
medianSteps <- tapply(act2$steps, as.numeric(act2$date), median) # THIS IS BROKEN
...
medianSteps2 <- tapply(act3$steps, as.numeric(act3$date), median) # THIS IS BROKEN
meanMedian2 <- cbind(rownames(table(as.character(act3$date))), meanSteps2, medianSteps2)
meanMedian2
...
median(act3$steps, na.rm = TRUE) - median(act$steps, na.rm = TRUE)
```
